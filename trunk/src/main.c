...
void createVFS(char *name, long size) {
	...
	printf("--------------------Creating VFS-----------------------------");
	b1.lfixed_size_buffer = 10240; // size of each file block = 10KB
	...
	m1.ltotal_file_descriptors = (m1.lsize)/(sizeof(fd1) + b1.lfixed_size_buffer); // calculating the no. of file descriptors can be used in remaining space
    ...
    b1.ltotal_file_blocks = (m1.lsize)/(b1.lfixed_size_buffer); // calculating the no. of file blocks can be used in remaining space
    printf("\nsize of each file block : %ld", b1.lfixed_size_buffer);
    printf("\ntotal file blocks : %ld", b1.ltotal_file_blocks);
    m1.lsize = m1.lsize - (b1.ltotal_file_blocks * b1.lfixed_size_buffer); // calculating the space unused 
	...
	if(fptr == NULL) {
		...
		for (; j<= b1.ltotal_file_blocks; j++) {
			fwrite(&b1, b1.lfixed_size_buffer, 1, fptr); // writing file blocks
		}
